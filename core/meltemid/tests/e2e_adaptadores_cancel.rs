// SPDX-License-Identifier: Apache-2.0

//! End-to-end cancellation through Meltemi's own ACP adapter
//! (adaptadores-propios-acp task 2.5), with the real daemon piloting the real
//! adapter binary and a scripted provider that **will not stop**.
//!
//! This lives in its own test binary because it needs its own scripted wire,
//! and a wire is selected by environment variable — which is per process.
//!
//! The scenario is the ugly one, the one worth an end-to-end test: the provider
//! opens a turn, says something, and then goes silent forever. Cancelling must
//! still end the session — the adapter interrupts the turn in the provider's own
//! terms and, when even that is ignored, ends the process rather than let a turn
//! that will never finish hold the session and its worktree open.

use std::path::PathBuf;
use std::time::Duration;

use serde_json::{Value, json};
use tokio::sync::mpsc;

use meltemi_client::rpc::Peer;
use meltemi_proto::{InitializeParams, PROTOCOL_VERSION, PeerInfo, methods};
use meltemid::server::{DaemonState, serve_until_shutdown};
use meltemid::transport::{Listener, connect};

/// A provider that opens a turn, speaks once, and then waits for a request that
/// never comes.
///
/// The last directive is what makes it hang: the scripted wire keeps reading,
/// acknowledging whatever the adapter asks — the interruption included — and
/// never ends the turn. Everything before it is the same shape the vendored
/// schema describes, and the conformance suite validates the fixture's lines
/// against that dump.
const HELD_OPEN: &str = r#"
{"mock":"await-request","method":"initialize"}
{"mock":"respond","result":{"userAgent":"codex_cli_rs/0.77.0 (held-open fixture) (meltemi-codex-acp; 0.1.0)"}}
{"mock":"await-request","method":"thread/start"}
{"mock":"respond","result":{"approvalPolicy":"on-request","cwd":"/mock/project","model":"mock-model","modelProvider":"mock","sandbox":{"type":"readOnly"},"thread":{"id":"mock-thread-1","cliVersion":"0.77.0","createdAt":1784000000,"cwd":"/mock/project","modelProvider":"mock","path":"/mock/threads/mock-thread-1.jsonl","preview":"held open","source":"appServer","turns":[]}}}
{"mock":"await-request","method":"turn/start"}
{"mock":"notify","method":"turn/started","params":{"threadId":"mock-thread-1","turn":{"id":"turn-1","status":"inProgress","items":[]}}}
{"mock":"notify","method":"item/agentMessage/delta","params":{"threadId":"mock-thread-1","turnId":"turn-1","itemId":"item-1","delta":"Thinking about it forever."}}
{"mock":"await-request","method":"nothing/ever/asks/this"}
"#;

fn workspace_bin(name: &str) -> PathBuf {
    let mut dir = std::env::current_exe().expect("current exe");
    dir.pop();
    if dir.ends_with("deps") {
        dir.pop();
    }
    let path = dir.join(if cfg!(windows) {
        format!("{name}.exe")
    } else {
        name.to_string()
    });
    assert!(
        path.exists(),
        "{name} was not built at {}; run `cargo test` at the workspace root",
        path.display()
    );
    path
}

fn test_endpoint(tag: &str) -> String {
    #[cfg(windows)]
    {
        format!(r"\\.\pipe\meltemid-e2e-{}-{tag}", std::process::id())
    }
    #[cfg(unix)]
    {
        std::env::temp_dir()
            .join(format!("meltemid-e2e-{}-{tag}.sock", std::process::id()))
            .to_string_lossy()
            .into_owned()
    }
}

/// A project whose agent is the real adapter, piloting the wire that will not
/// stop.
fn fixture() -> PathBuf {
    let adapter = workspace_bin("meltemi-codex-acp");
    let root = std::env::temp_dir().join(format!(
        "meltemi-e2e-adapters-cancel-{}",
        std::process::id()
    ));
    let _ = std::fs::remove_dir_all(&root);
    std::fs::create_dir_all(root.join(".meltemi")).unwrap();

    let script = root.join("held-open.ndjson");
    std::fs::write(&script, HELD_OPEN.trim_start()).unwrap();
    // SAFETY: this test binary runs one test, and sets these before any adapter
    // is launched. The adapter inherits them through the daemon, which is the
    // same inheritance a real launch uses.
    unsafe {
        std::env::set_var("MELTEMI_CODEX_BIN", workspace_bin("mock-codex-wire"));
        std::env::set_var("MELTEMI_MOCK_CODEX_SCRIPT", &script);
        std::env::remove_var("MELTEMI_AGENT_COMMAND");
    }

    std::fs::write(
        root.join(".meltemi").join("config.toml"),
        format!(
            "[agent]\ncommand = ['{}']\n",
            adapter.display().to_string().replace('\\', "/")
        ),
    )
    .unwrap();
    root
}

async fn client(endpoint: &str) -> Peer {
    let stream = connect(endpoint).await.expect("connect");
    let (peer, mut incoming) = Peer::start(stream);
    tokio::spawn(async move { while incoming.recv().await.is_some() {} });
    peer.request(
        methods::INITIALIZE,
        &InitializeParams {
            protocol_version: PROTOCOL_VERSION,
            client: PeerInfo {
                name: "e2e-adapters-cancel".into(),
                version: "0".into(),
            },
        },
    )
    .await
    .expect("initialize");
    peer
}

/// The session's log, as parsed events.
async fn session_events(peer: &Peer, root: &str) -> Vec<Value> {
    let list = peer
        .request(methods::SESSION_LIST, &json!({ "projectRoot": root }))
        .await
        .expect("session/list ok");
    let Some(id) = list["sessions"][0]["sessionId"].as_str() else {
        return Vec::new();
    };
    let log = peer
        .request(
            methods::SESSION_LOG,
            &json!({ "projectRoot": root, "sessionId": id, "limit": 1000 }),
        )
        .await
        .expect("session/log ok");
    log["lines"]
        .as_array()
        .expect("the log's raw lines")
        .iter()
        .map(|line| {
            serde_json::from_str(line.as_str().expect("a JSONL line"))
                .expect("every logged line is JSON")
        })
        .collect()
}

#[tokio::test]
async fn cancelling_ends_a_turn_the_provider_refuses_to_end() {
    // Scenario: Cancelación propagada al servidor
    let root = fixture();
    let root_str = root.display().to_string();

    let endpoint = test_endpoint("adapters-cancel");
    let listener = Listener::bind(&endpoint).await.expect("bind");
    let (shutdown_tx, shutdown_rx) = mpsc::channel(1);
    let state = DaemonState::for_test("adapters-cancel", shutdown_tx);
    let daemon = tokio::spawn(serve_until_shutdown(listener, state, shutdown_rx));
    let peer = client(&endpoint).await;

    let proposing = tokio::spawn({
        let peer = peer.clone();
        let root_str = root_str.clone();
        async move {
            peer.request(
                methods::PROPOSE,
                &json!({ "idea": "a turn that will not end", "projectRoot": root_str }),
            )
            .await
        }
    });

    // Cancel only once the turn is demonstrably in flight — the prompt has gone
    // out and the provider has spoken *in that turn*. Cancelling into a session
    // whose turn has not started is answered by "there is nothing to
    // interrupt", which is correct and proves nothing: the scripted wire then
    // holds its turn open forever and the test dies of its own timeout.
    //
    // Asking for any agent update is not that question and stopped being it
    // when the session's provenance became one: that update is emitted at
    // `session/new`, before the prompt exists. A message chunk is the provider
    // speaking inside the turn, which is the fact this test needs.
    let mut session_id = None;
    for _ in 0..200 {
        let events = session_events(&peer, &root_str).await;
        if events.iter().any(|event| event["type"] == "prompt_sent")
            && events
                .iter()
                .any(|event| event["payload"]["update"]["sessionUpdate"] == "agent_message_chunk")
            && let Some(started) = events
                .iter()
                .find(|event| event["type"] == "session_started")
        {
            session_id = started["payload"]["sessionId"].as_str().map(str::to_string);
            break;
        }
        tokio::time::sleep(Duration::from_millis(50)).await;
    }
    let session_id = session_id.expect("the turn started and the provider spoke");

    peer.notify(methods::SESSION_CANCEL, &json!({ "sessionId": session_id }));

    // The turn ends. Not because the provider agreed to end it — it never does —
    // but because the adapter stops waiting on a provider that ignored its
    // interruption, and ends it.
    let outcome = match tokio::time::timeout(Duration::from_secs(60), proposing).await {
        Ok(joined) => joined.expect("the task ran"),
        Err(_) => {
            let events = session_events(&peer, &root_str).await;
            panic!("the cancelled turn hung; the session log says: {events:#?}");
        }
    };
    assert!(outcome.is_ok(), "propose answered: {outcome:?}");

    let events = session_events(&peer, &root_str).await;
    let kinds: Vec<&str> = events
        .iter()
        .filter_map(|event| event["type"].as_str())
        .collect();
    assert!(
        kinds.contains(&"turn_completed") || kinds.contains(&"session_ended"),
        "the session closed rather than hanging: {kinds:?}"
    );

    // Nothing lists as running any more: the provider process went with it.
    let list = peer
        .request(methods::SESSION_LIST, &json!({ "projectRoot": root_str }))
        .await
        .expect("session/list ok");
    assert!(
        list["sessions"]
            .as_array()
            .expect("sessions")
            .iter()
            .all(|session| session["state"] != "active"),
        "no session is left running: {list:#}"
    );

    peer.close();
    daemon.abort();
    let _ = std::fs::remove_dir_all(&root);
}
