...
`version`, `help`) y los subcomandos del ciclo SDD reservados (`explore`,
`review`, `plan`, `implement`, `verify`, `archive`). Un subcomando reservado MUST
reconocerse como parte de la gramática y no como error de uso.
...
`stop`→`shutdown`, `fleet`→`fleet/list`, `project`→`context/project`. Los
subcomandos locales (`version`, `help`) MUST NOT abrir conexión con el daemon.
...
- **WHEN** se invoca `meltemi project` con el daemon accesible
- **THEN** el binario SHALL invocar el método `context/project` y presentar los destinos escritos
