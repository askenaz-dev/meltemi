...
subcomandos operativos (`status`, `propose`, `stop`, `fleet`, `version`, `help`)
y los subcomandos del ciclo SDD reservados (`explore`, `review`, `plan`,
`implement`, `verify`, `archive`). Un subcomando reservado MUST reconocerse como
parte de la gramática y no como error de uso.
...
`stop`→`shutdown`, `fleet`→`fleet/list`. Los subcomandos locales (`version`,
`help`) MUST NOT abrir conexión con el daemon.
...
#### Scenario: fleet consulta el catálogo
- **WHEN** se invoca `meltemi fleet` con el daemon accesible
- **THEN** el binario SHALL invocar el método `fleet/list` y presentar el catálogo
