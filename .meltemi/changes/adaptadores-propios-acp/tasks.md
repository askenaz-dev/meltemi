...
- [ ] 5.3 El dialecto headless abre sesión contra el CLI real: el evento inicial se lee cuando el CLI lo emite —después de la primera entrada, no antes— y la identidad de sesión deja de depender de él; los guiones del cable simulado se corrigen a ese orden y la corrida manual vuelve a 5.2 para re-verificar
  <!-- 2026-07-28: abierta por la corrida manual de 5.2 (docs/conformidad-manual.md, enmienda D4). Contra `claude` 2.1.167 toda sesión rehúsa con `provider_handshake_failed` tras 60 s porque el adaptador espera el evento inicial antes de enviar nada y el CLI no lo emite hasta recibir entrada. Lo que hace falta: (a) diferir la lectura del evento inicial al primer turno, después de escribir el mensaje de usuario, aplicando ahí la guarda de superficie antes de mapear nada; (b) dictar la identidad de sesión con `--session-id <uuid>` (existe en 2.1.167) en vez de aprenderla del evento inicial, que es lo que hoy ata el handshake al `session/new` y sostiene la reanudación de 3.5; (c) mover el evento inicial detrás del primer `await-input` en `core/mock-provider/scripts/claude-*.ndjson` y en los guiones de los e2e y de la suite de conformidad, y quitar el arreglo `capabilities` que el CLI real no emite; (d) volver a correr 5.2 para el dialecto headless y actualizar la sección «Última corrida». -->
