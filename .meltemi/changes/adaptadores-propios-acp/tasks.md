...
- [x] 5.2 Corrida de conformidad manual contra los CLIs reales documentada (instrucciones opt-in por plataforma), resultado persistido con fecha y versión, y escenarios solo-manuales marcados vía verify-mark con nota
  <!-- 2026-07-31, revisión adversarial: la corrida gana una segunda pierna por dialecto (una sesión que se para en cuanto el CLI habla dentro de su turno), porque con una sola el nivel 2 era inalcanzable por construcción — `cancellation` no se ejercía nunca. Resultado: `claude` 2.1.167 queda en **nivel verificado 2**, los cuatro criterios ejercidos y aprobados; `codex` 0.77.0 queda en 0 y no por el puente — la corrida destapó primero que el CLI no se podía lanzar en Windows (npm deja `.cmd` y `CreateProcess` sólo añade `.exe`; corregido, design D14) y después que el modelo por defecto de la cuenta exige un Codex más nuevo que el 0.77.0 anclado, de modo que todo turno vuelve rechazado por el proveedor. La pierna del paro se esperaba barata y medida no lo es: cuesta prácticamente lo mismo que la del turno completo. Todo, con números y por dialecto, en docs/conformidad-manual.md. -->
  <!-- 2026-07-31: marcado además vía verify-mark el escenario de la compuerta dura, cuya mitad de proveedor —que un CLI en modo permisivo corra su hook y obedezca la denegación— sólo la puede observar el binario oficial; la mitad de Meltemi la prueba `core/meltemid/tests/e2e_adaptadores_claude_bypass.rs` contra un cable que anuncia ese modo. -->
