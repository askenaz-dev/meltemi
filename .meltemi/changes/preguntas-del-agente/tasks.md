...
- [x] 2.1 Bandera **apagada por defecto** (como `--honor-cancel`): con ella, el
...
- [x] 3.1 Con la sesión esperando decisión, el compositor presenta la petición y
...
- [x] 3.2 Teclado: recorrer con flechas, elegir con Enter, y el foco donde el
  usuario ya está mirando
- [x] 3.3 Aparece **sin animación de layout** — la regla vigente es literal
  (`gui-shell/spec.md:266-268`) — escenario «La pregunta aparece sin mover nada»
- [x] 3.4 Tope visual: el listado se desplaza **dentro de sí mismo**, nunca el
  panel (design D4) — escenario «Muchas opciones se desplazan dentro del
  listado»
- [x] 3.5 La tarjeta del transcript **se conserva**: el registro es la verdad y
  la tarjeta es su lectura

## 4. La salida de texto libre, con la verdad de cada cable

- [x] 4.1 Última opción «Otra respuesta…» que abre la caja en el mismo sitio
- [x] 4.2 Su rótulo dice qué hará **según el agente de la sesión**: responder la
  ...
  <!-- 2026-08-16: **el design D5 estaba equivocado y se corrigió aquí.** El
  texto libre no puede llegar al adaptador de Claude tampoco: el tramo
  daemon→adaptador es ACP en los dos casos, y un desenlace ACP es
  `Selected(optionId)` o `Cancelled` (`claude/mod.rs:576-582`). No hay campo por
  donde viaje, así que no hay nada que poner en `updatedInput`. Un solo
  comportamiento, el honesto: se interrumpe el turno y el texto lo releva. -->
- [x] 4.3 i18n es/en de todo lo nuevo, con el lint como guardián
